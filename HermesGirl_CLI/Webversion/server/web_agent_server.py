...
LEGACY_ROLE_MAP = {"user": "student", "admin": "teacher"}
...
def require_path_inside(path: str, root: str, label: str = "path") -> str:
    path = os.path.abspath(path)
    if not is_path_inside(path, root):
        raise PermissionError(f"{label} is outside the current user's folder")
    return path
...
class UserManager:
    ...
    def _load_unlocked(self) -> Dict:
        ...
        for record in data["users"].values():
            if isinstance(record, dict):
                record["role"] = normalize_user_role(record.get("role", "student"))
        return data
    ...
    def ensure_bootstrap_admin(self) -> Dict:
        with self.lock:
            ...
            for record in users.values():
                if normalize_user_role(record.get("role")) == "teacher" and record.get("status") == "active":
                    return {}

            username = normalize_username(
                os.environ.get("ANNA_WEB_TEACHER_USER")
                or os.environ.get("ANNA_WEB_ADMIN_USER", "teacher")
            )
            try:
                validate_username(username)
            except ValueError:
            ...
            password = (
                os.environ.get("ANNA_WEB_TEACHER_PASSWORD", "").strip()
                or os.environ.get("ANNA_WEB_ADMIN_PASSWORD", "").strip()
            )
            generated = False
            if not password:
                generated = True
            ...
            if generated:
                with open(BOOTSTRAP_ADMIN_PATH, "w", encoding="utf-8") as f:
                    f.write(
                        ...
                        f"username={username}\n"
                        f"password={password}\n"
                        "Change this password by creating a new teacher account in the management page.\n"
                    )
            return {
                "username": username,
                "password": password,
    ...
    def register(self, username: str, password: str) -> Dict:
        return self.create_user(username, password, role="student", status="pending")

    def create_user(self, username: str, password: str, role: str = "student", status: str = "active") -> Dict:
        username = normalize_username(username)
        validate_username(username)
    ...
    def list_users(self) -> List[Dict]:
        ...
        return sorted(users, key=lambda item: (item.get("role") != "teacher", item.get("username", "")))
    ...
    def public_user(self, record: Dict) -> Dict:
        return {
            ...
            "role": normalize_user_role(record.get("role", "student")),
...
class ServerFileRegistry:
    ...
    def upsert_upload(
        ...
        server_path = require_path_inside(server_path, self.files_dir, "server file path")
        now = time.time()
    ...
    def forget(self, key: str, rag: RagContextManager):
        ...
        if server_path and not is_path_inside(server_path, self.files_dir):
            raise PermissionError("stored server path is outside this user's folder")
    ...
    def unique_server_path(self, key: str, filename: str) -> str:
        ...
        require_path_inside(directory, self.files_dir, "upload directory")
        ...
        while os.path.exists(candidate):
            ...
            suffix += 1
        return require_path_inside(candidate, self.files_dir, "upload path")
...
class UserServerState:
    def __init__(self, username: str):
        ...
        require_path_inside(self.root_dir, USER_DATA_DIR, "user folder")
...
def build_admin_page() -> str:
    ...
    if bootstrap.get("generated"):
        setup_notice = (
            "<p class='notice'>A bootstrap teacher account was created. Credentials were written to "
...
<body>
  <main>
    <header>
      <div>
        ...
        <p>Approve student registrations and create teacher accounts from this management page.</p>
      </div>
    ...
    <section id="users-panel" class="hidden">
      ...
      <div class="panel">
        ...
        <p>Client registrations are always students. Teacher accounts can only be created here.</p>
        <div class="row">
          <input id="new-username" autocomplete="off" placeholder="Username">
          <input id="new-password" type="password" autocomplete="new-password" placeholder="Password">
          <select id="new-role">
            <option value="student">Student</option>
            <option value="teacher">Teacher</option>
          </select>
          <button id="create-user" class="primary">Create Active User</button>
        </div>
      </div>
      <table>
        <thead>
  ...
  <script>
    ...
    async function createUser() {{
      ...
      setStatus("Creating " + role + " " + username + "...");
    ...
    document.getElementById("create-user").onclick = () => createUser().catch(err => alert(err.message));
...
class WebAgentHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/rag/sources":
            ...
            prune_user_rag_outside_folder(user_state)
            return self.send_json({"sources": user_rag_sources(user_state)})
            ...
            return self.send_json(
                {
                    "ok": True,
                    "status": "pending",
                    "message": "Student registration is pending teacher approval.",
                    "user": user,
                },
                status=202,
            )
    ...
    def handle_rag_source(self):
        ...
        user_state = self.user_state()
        if not is_path_inside(source, user_state.root_dir):
            return self.send_json({"error": "source is outside this account's folder"}, status=403)
        chunks = user_state.rag.store.get_source_chunks(source)
        if not chunks:
    ...
    def handle_file_upload(self):
        key = safe_storage_key(self.headers.get("X-File-Key", "").strip() or uuid.uuid4().hex)
        ...
        try:
            ...
            if old_path and old_path != server_path:
                try:
                    if is_path_inside(old_path, user_state.files_dir) and os.path.isfile(old_path):
                        os.unlink(old_path)
    ...
    def require_admin(self) -> bool:
        return self.require_teacher()

    def require_teacher(self) -> bool:
        if self.authenticate() and normalize_user_role(self.current_user_record.get("role")) == "teacher":
            return True
        self.send_json({"error": "teacher authorization required"}, status=403)
        return False
...
def index_uploaded_file(path: str, user_state: UserServerState) -> tuple:
    path = require_path_inside(path, user_state.files_dir, "indexed file")
    try:
        with open(path, "rb") as f:
            data = f.read()
...
def reindex_file_record(record: Dict, user_state: UserServerState) -> Dict:
    ...
    if server_path and not is_path_inside(server_path, user_state.files_dir):
        result = {
            "key": key,
            "filename": filename,
            "ok": False,
            "rag_indexed": False,
            "index_status": "Server file path is outside this account's folder and was blocked.",
        }
...
def user_rag_sources(user_state: UserServerState) -> List[Dict]:
    return [
        source
        for source in user_state.rag.store.list_sources()
        if is_path_inside(source.get("source", ""), user_state.root_dir)
    ]


def prune_user_rag_outside_folder(user_state: UserServerState) -> int:
    deleted = 0
    for source in user_state.rag.store.list_sources():
        path = source.get("source", "")
        if path and not is_path_inside(path, user_state.root_dir):
            user_state.rag.store.delete_source(path)
            deleted += 1
    return deleted


def read_user_memory(user_state: UserServerState) -> str:
    require_path_inside(user_state.memory_path, user_state.root_dir, "memory path")
...
def copy_server_files(package_dir: str, user_state: UserServerState):
    ...
    for record in user_state.registry.list_files():
        ...
        key = safe_storage_key(record.get("key") or uuid.uuid4().hex)
...
def restore_server_rag_files(extract_dir: str, user_state: UserServerState) -> tuple:
    ...
    for item in manifest.get("files", []):
        ...
        if not is_path_inside(archive_file, extract_dir):
            continue
        if not os.path.isfile(archive_file):
            continue

        key = safe_storage_key(item.get("key", "") or uuid.uuid4().hex)
...
def merge_server_rag_database(
    ...
    with sqlite3.connect(imported_db_path) as imported, sqlite3.connect(user_state.rag_db_path) as target:
        ...
        for source, chunk_index, text, summary, vector_json in cursor.fetchall():
            ...
            if not new_source:
                new_source = basename_map.get(os.path.basename(source or ""), "")
            if not new_source or not is_path_inside(new_source, user_state.files_dir):
                continue
...
def merge_server_file_log(
    ...
    for key, record in (imported_data.get("files") or {}).items():
        if not isinstance(record, dict):
            continue
        key = safe_storage_key(record.get("key") or key or uuid.uuid4().hex)
        new_record = dict(record)
        new_record["key"] = key

        mapped_path = key_path_map.get(key) or source_path_map.get(record.get("server_path", ""))
        if mapped_path:
            mapped_path = require_path_inside(mapped_path, user_state.files_dir, "imported file path")
...
def find_key_for_imported_path(path: str, current_files: Dict[str, Dict]) -> str:
    ...
    for key, record in current_files.items():
        server_path = record.get("server_path", "")
        if server_path and os.path.abspath(server_path) == path:
            return key
    return ""
...
def resolve_backup_name(name: str, user_state: UserServerState) -> str:
    ...
    if not is_path_inside(path, backups_dir):
        return ""
    if not os.path.isfile(path):
        return ""
    return path
...
def load_requested_server_file(key: str, user_state: UserServerState) -> Dict:
    ...
    if not is_path_inside(server_path, files_root):
        return {
            "key": key,
            "filename": filename,
            "summary": summary,
...
def delete_server_file(record: Dict, user_state: UserServerState):
    ...
    server_path = record.get("server_path", "")
    if server_path and not is_path_inside(server_path, user_state.files_dir):
        user_state.registry.mark_deleted(key, "stored path was outside this user's folder")
        return
    if server_path and os.path.isfile(server_path):
        os.unlink(server_path)
    if server_path:
    ...
    files_root = os.path.abspath(user_state.files_dir)
    if parent and is_path_inside(parent, files_root) and os.path.isdir(parent):
        try:
...
def main():
    ...
    print(f"Management UI: http://{HOST}:{PORT}/management")
    print(f"Users: {USERS_DB_PATH}")
    if STATE.bootstrap_admin.get("generated"):
        print(
            "Bootstrap teacher created: "
            f"{STATE.bootstrap_admin.get('username')} / {STATE.bootstrap_admin.get('password')}"
        )
        print(f"Bootstrap teacher file: {STATE.bootstrap_admin.get('setup_path')}")
