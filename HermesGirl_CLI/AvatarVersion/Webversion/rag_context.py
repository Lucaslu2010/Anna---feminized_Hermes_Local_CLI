    ...
    def add_file_text(self, name: str, text: str) -> str:
        ...
        if not self.client.is_configured():
            ...
            detail = (
                ...
                f"exists: {diagnostics.get('config_exists')}; "
                f"key present: {diagnostics.get('embedding_api_key_present')}; "
                f"key length: {diagnostics.get('embedding_api_key_length')}"
            )
            if diagnostics.get("config_error"):
                detail += f"; config error: {diagnostics.get('config_error')}"
            return (
