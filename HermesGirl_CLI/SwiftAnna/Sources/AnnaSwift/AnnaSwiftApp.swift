    ...
    private var rootContent: some View {
        if !model.didLoadConfig {
            ...
            ZStack(alignment: .bottomTrailing) {
                HStack(spacing: 0) {
                    ChatManagementSidebar(model: model)
                    ChatColumn(model: model)
                    LearningSidebar(model: model)
                }
...
final class AnnaModel: ObservableObject {
    @Published var config = BridgeConfig(webModeEnabled: false, serverURL: "http://127.0.0.1:8765", username: "", role: "", locationInjectionEnabled: false, signedIn: false)
    @Published var didLoadConfig = false
    @Published var chatSessions: [ChatSession]
    @Published var activeChatID: UUID
    @Published var chatSidebarCollapsed = false
    ...
    init() {
        let firstChat = ChatSession()
        self.chatSessions = [firstChat]
        self.activeChatID = firstChat.id
        self.messages = firstChat.messages
    }
    ...
    func updateActiveChat() {
        guard let index = chatSessions.firstIndex(where: { $0.id == activeChatID }) else { return }
        ...
        if let firstUserMessage = messages.first(where: { $0.role == "user" })?.content {
            chatSessions[index].title = chatTitle(from: firstUserMessage)
        }
    }
...
struct ChatManagementSidebar: View {
    @ObservedObject var model: AnnaModel

    var body: some View {
        Group {
            if model.chatSidebarCollapsed {
                VStack(spacing: 12) {
                    Button {
                        withAnimation(.spring(response: 0.25, dampingFraction: 0.86)) {
                            model.chatSidebarCollapsed = false
                        }
                    } label: {
                        Image(systemName: "sidebar.left")
                    }
                    .buttonStyle(IconButtonStyle())
                    .accessibilityLabel("Expand chats")
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        ...
                        Button {
                            withAnimation(.spring(response: 0.25, dampingFraction: 0.86)) {
                                model.chatSidebarCollapsed = true
                            }
                        } label: {
                            Image(systemName: "sidebar.left")
                        }
                    ...
                    ScrollView {
                        LazyVStack(spacing: 8) {
                            ForEach(model.chatSessions) { session in
                                ChatSessionRow(
                                    session: session,
                                    isActive: session.id == model.activeChatID,
                                    select: { model.selectChat(session) },
                                    delete: { model.deleteChat(session) }
                                )
                            }
                        }
        ...
        .animation(.spring(response: 0.25, dampingFraction: 0.86), value: model.chatSidebarCollapsed)
    }
}
...
struct ChatSessionRow: View {
    ...
    var body: some View {
        Button(action: select) {
            HStack(spacing: 8) {
                Image(systemName: "message")
                    .foregroundStyle(isActive ? AnnaTheme.accent : .secondary)
                    .frame(width: 18)
            ...
            .background(isActive ? Color.white : Color.white.opacity(0.68))
            .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
            .contentShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(isActive ? AnnaTheme.accent.opacity(0.45) : Color.black.opacity(0.05))
            )
        }
        .buttonStyle(.plain)
    }
        ...
        VStack(alignment: .leading, spacing: 18) {
            VStack(alignment: .leading, spacing: 6) {
                Text(model.config.signedIn ? model.config.username : "Anna")
                    ...
                    .lineLimit(1)
                Text(model.config.role.isEmpty ? "Learning workspace" : model.config.role.capitalized)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            .padding(.top, 34)
...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text(model.config.signedIn ? "Signed in as \(model.config.username) \(model.config.role.isEmpty ? "" : "(\(model.config.role))")" : "Not signed in")
                .font(.callout)
                .foregroundStyle(.secondary)
...
func chatTitle(from text: String) -> String {
    let words = text
    ...
    return words.count > 36 ? String(words.prefix(36)) + "..." : words
}
